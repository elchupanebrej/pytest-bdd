...
def test_e2e_parsing_feature_with_examples_1(resource_path):
    feature_file_yaml_path = (resource_path / ".." / ".." / "feature_with_examples.feature.yaml").resolve()
    ...
    feature_example_csv_path = (resource_path / ".." / ".." / "simple_feature_example.csv").resolve()
    feature_example_csv_data = feature_example_csv_path.read_text()

    get_mock = Mock()
    get_mock.text = feature_example_csv_data.strip()
    with patch("pytest_bdd.model.feature.builder.get", return_value=get_mock):
